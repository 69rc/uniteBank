  ...
  return (
    <Layout>
      <div className="flex flex-col md:flex-row gap-8 items-start justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-slate-900">Financial Overview</h1>
          <p className="text-slate-500">Welcome back, {user?.firstName}</p>
        </div>
        <div className="text-right">
          <p className="text-sm font-medium text-slate-400 uppercase tracking-wider">Current Balance</p>
      ...
      {/* Account Card */}
      <div className="relative overflow-hidden rounded-3xl bg-slate-900 text-white shadow-2xl p-8 h-64 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-start z-10">
          <div>
            ...
            <p className="text-xl font-semibold">Premium Savings</p>
          </div>
        ...
        <div className="z-10">
          ...
          <p className="font-mono text-3xl tracking-widest">{user?.accountNumber?.replace(/(.{4})/g, '$1 ')}</p>
        </div>
      </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {transactions?.length === 0 ? (
                   <tr>
                     <td colSpan={4} className="text-center py-8 text-slate-500">No transactions found</td>
                   </tr>
                ) : (
                  transactions?.map((tx) => (
                    <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0">
                      ...
                      <td className={`text-right py-4 px-4 font-bold font-mono ${
